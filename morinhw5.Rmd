...
### Create folds makes indexes the observations left out
test = createFolds(iod.clean$GFR, k = folds)
...
ridge.plot.frame %>% ggplot(aes(x = lambda, y = error)) +
  geom_point() +
  geom_line() +
  theme_classic() +
  theme(text=element_text(size=12,  family="CM Sans"))
...
est.ridge.optimism[which.min(ridge.plot.frame$error)]
...
est.lasso.optimism[which.min(lasso.plot.frame$error)]

...
### Filter out ids
iod.clean = iod.clean %>%
  select(-X1, -ID, -"..2", -"..2_1", -WEIGHT, -HEIGHT, -c.cys, -drds, -mayodonor, -rass1, -CSG, -gronigendonor, - ccpdonor, - c.scr)
...
cv.errors = matrix(NA, nrow = folds, ncol = ncol(iod.clean) - 2)
...
for (i in 1:folds) {
  ...
  best.model = regsubsets(GFR ~ .,
                          data = training,
                          nvmax = ncol(iod.clean) - 2)
  
  for (j in 1:(ncol(iod.clean)-2)) {
